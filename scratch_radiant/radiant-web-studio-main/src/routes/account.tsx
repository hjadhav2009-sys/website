    ...
    <div className="min-h-screen bg-offwhite flex flex-col">
      ...
      <main className="flex-1 flex items-center justify-center py-16 px-4">
        <div className="bg-white rounded-3xl border border-[var(--color-border)] shadow-brand-md p-8 w-full max-w-md">
          {/* Tabs */}
          <div className="flex border border-[var(--color-border)] rounded-xl p-1 mb-8">
            {(["signin", "register"] as const).map((t) => (
              <button
                key={t}
                onClick={() => setTab(t)}
                className={`flex-1 py-2.5 rounded-lg text-sm font-semibold transition-all ${
                  tab === t ? "bg-brand text-white shadow-brand-sm" : "text-muted-foreground hover:text-ink"
                }`}
              >
          ...
          {tab === "signin" ? (
            <form onSubmit={(e) => e.preventDefault()} className="space-y-4">
              ...
              <div className="relative text-center text-sm text-muted-foreground my-2">
              ...
              <button type="button" className="w-full border-2 border-[var(--color-border)] bg-white text-sm font-semibold py-3 rounded-xl hover:border-brand hover:bg-brand-pale transition flex items-center justify-center gap-2">
            ...
            <form onSubmit={(e) => e.preventDefault()} className="space-y-4">
              ...
              <div>
                <label className="label-eyebrow block mb-1.5">Email</label>
                <input type="email" required placeholder="your@email.com" className="w-full border border-[var(--color-border)] rounded-xl px-4 py-3 text-sm outline-none focus:border-brand transition" />
              </div>
              <div>
                <label className="label-eyebrow block mb-1.5">Password</label>
              ...
              <div>
                <label className="label-eyebrow block mb-1.5">WhatsApp Number</label>
                <input type="tel" required placeholder="+91 98765 43210" className="w-full border border-[var(--color-border)] rounded-xl px-4 py-3 text-sm outline-none focus:border-brand transition" />
              </div>
              <label className="flex items-start gap-2 text-sm cursor-pointer">
                <input type="checkbox" required className="accent-brand mt-0.5" />
                <span className="text-muted-foreground">I agree to the <Link to="/terms" className="text-brand hover:underline">Terms</Link> &amp; <Link to="/privacy-policy" className="text-brand hover:underline">Privacy Policy</Link></span>
              </label>
              <button type="submit" className="w-full bg-brand text-white font-bold py-3.5 rounded-xl hover:bg-brand-dark transition">
                Create Account
              </button>
